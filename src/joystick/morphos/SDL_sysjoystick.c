				...
				if (GetSensorAttrTags(sensor, SENSORS_Type, (IPTR)&type, TAG_DONE))
				{
					switch (type) 
					{
						...
						case SensorType_HIDInput_Rumble:
							...
							{
								GetSensorAttrTags(sensor, SENSORS_HID_Name, (IPTR)&name, TAG_DONE);
								D("SensorType_HIDInput_Rumble SensorType: %d name: %s", type, name);
...
static bool MORPHOS_JoystickRumble(SDL_Joystick * joystick, Uint16 low_frequency_rumble, Uint16 high_frequency_rumble)
{
	...
	if (hwdata) 
	{
		if (hwdata->numRumbles)
		{
			...
			ULONG duration = SDL_MAX_RUMBLE_DURATION_MS; //duration_ms;
...
{
	/*Additionally, xbox one gamepads will now report 4 motors. You can discover which motor those are by querying for SENSORS_HIDInput_Limb and SENSORS_HIDInput_Rumble_Type, but generally:
1 - vibration left,
2 - vibration right,
3 - impulse left,
4 - impulse right
So you can now implement JoystickRumbleTriggers using the impulse motors
